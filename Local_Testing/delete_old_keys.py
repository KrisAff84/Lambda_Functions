"""
This script generates a credential report, determines which access keys are older than 
a specified number of days, and marks those keys for deletion. It then sends an SNS message to a 
specified topic with the keys that were deleted. If no keys were determined to be older than the 
specified number of days, the SNS message will indicate that the function was successful but no keys were
deleted.
...
if __name__ == '__main__':
    max_number_of_days_str = "180"
    max_number_of_days = int(max_number_of_days_str)
    sns_topic_arn = "arn:aws:sns:us-east-1:835656321421:Delete_Old_Access_Keys"
